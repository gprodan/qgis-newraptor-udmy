...
class NewRaptor:
    ...
    def run(self):
        ...
        #QMessageBox.information(self.dlg, "Message", "Should only run every time")
        ...
        if result:
            ...
            valBuffer = self.dlg.spbBufferDistance.value()
            ...
            dlgTable = DlgTable()
            dlgTable.setWindowTitle("Impact Table for Nest {}".format(valNestID))
            # Find linear projects that will be imapcted and report them in the table
    ...
    def evt_cmbSpecies_changed(self, species):
        if species == "Swainsons Hawk":
            self.dlg.spbBufferDistance.setValue(0.004)
        else:
            self.dlg.spbBufferDistance.setValue(0.008)